...
public class Main extends Application {
    ...
    private static void runCLI() {
        ...
        Room amphi   = new Room("Amphi A", 200, 1);
        Room salle   = new Room("Salle 101", 30, 1);
        Passage couloir = new Passage("Couloir", 50, 1, 1.0, PassageType.CORRIDOR, 15.0);
        Exit exit = new Exit("Sortie", 100);

        Door d1 = new Door(amphi, couloir);
        Door d2 = new Door(salle, couloir);
        amphi.addDoor(d1); salle.addDoor(d2);
        couloir.addDoor(d1); couloir.addDoor(d2);

        campus.addElement(amphi); campus.addElement(salle);
        campus.addElement(exit); campus.addPassage(couloir);

        Person p = new Person("Test", amphi, 1.0, Behavior.POLITE, 0.7);
        p.setDestination(exit);
        campus.addAgent(p);

        List<BuildingElement> path = PathFinder.calculateShortestPath(amphi, exit);
